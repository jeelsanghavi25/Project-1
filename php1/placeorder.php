<?php
 ...
 $amt=0;
  ...
  $sql1="select * from fruits where pid=".$pid."";
...
<input type="hidden" name="acc_no" value=""> <br>
	<?php echo "<a href='addorder.php?amt=".$amt."'> <input class='btn btn-primary' value='Confirm Order' type='submit' style='background-color: #000000; border:#000000;'></a>"; ?><?php if($res=="true") {?> <div> <br><p  style="color:white;">Your Order has been Placed.</p></div> <?php }?>
...
<input class='btn btn-primary' value='Proceed to payment' type='submit' style='background-color: #000000; border:#000000;' id='isVisible'>
        ...
        <form class="form-horizontal" role="form" method="POST" action="cart.php">
          <input class="btn btn-primary" type ="submit" style="background-color: #000000; border:#000000;margin-top:20px;" value="Cancel">
        </div>
      </div>
  ...
  <div class="col-lg-7">
     ...
     <?php
              ...
              if(isset($_SESSION["username"])){
                ...
                if($result->num_rows>0){
                  while ($rss = mysqli_fetch_assoc($result)){
                    $sql = "select * from fruits where pid=".$rss["pid"].";";
